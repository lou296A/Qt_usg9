...
WidgetTopDown::WidgetTopDown(QWidget *parent)
    : QWidget(parent)
{
}


void WidgetTopDown::setData(const QList<XYZ>& newXYZList, const QList<short>& newIntensityList) {
...
void WidgetTopDown::paintEvent(QPaintEvent *event)
{
...
void WidgetTopDown::paint(QPainter *painter, QPaintEvent *event, const QList<XYZ>& xyzList, const QList<short>& intensityList)
{
    ...
    for (int i = 0; i < xyzList.size(); ++i) {
        ...
        // Draw the voxel
        if (intensity > 0) {
            painter->drawRect(QRectF(QPointF(8*xyz.x, 8*xyz.z), QSizeF(4, 4)));
        }
    }
